...
def create_dataset(username):
	...
	predictor = dlib.shape_predictor('recognition/haarcascade_frontalcatface.xml')   #Add path to the shape predictor ######CHANGE TO RELATIVE PATH LATER
...
def update_attendance_in_db_in(present):
    ...
    for person in present:
        # Gunakan filter untuk memastikan user ada
        user = User.objects.filter(username=person).first()
        if not user:
            print(f"User {person} tidak ditemukan.")
            continue

        # Cek apakah sudah ada absensi masuk pada tabel Present untuk hari ini
        try:
            qs = Present.objects.get(user=user, date=today)
        except Present.DoesNotExist:
            qs = None

        # Pastikan tidak ada duplikasi absensi masuk
        time_logs = Time.objects.filter(user=user, date=today, out=False)

        if qs is None:
            # Jika tidak ada absensi pada tabel Present untuk hari ini
            if present[person]:
                # Simpan absensi pada tabel Present
                a = Present(user=user, date=today, present=True, time_in=time)
                if time.hour < 7:
                    a.status_in = "Datang Tepat Waktu"
                else:
                    a.status_in = "Datang Terlambat"
                a.save()

                # Simpan waktu masuk ke tabel Time
                if not time_logs.exists():  # Periksa apakah belum ada data di Time
                    time_entry = Time(user=user, date=today, time=time, out=False)
                    time_entry.save()
        else:
            # Jika sudah ada absensi masuk pada tabel Present
            if present[person]:
                if not time_logs.exists():  # Periksa apakah belum ada data masuk di Time
                    # Simpan waktu masuk ke tabel Time
                    time_entry = Time(user=user, date=today, time=time, out=False)
                    time_entry.save()
                else:
                    # Jika sudah ada absensi masuk pada tabel Time
                    error_message = f"{user.username} sudah absen masuk hari ini."
...
def update_attendance_in_db_out(present):
    ...
    for person in present:
        ...
        if not user:
            print(f"User {person} tidak ditemukan.")
            continue  # Lewati ke user berikutnya jika tidak ditemukan

        # Cek apakah sudah ada absensi masuk pada hari ini
        try:
            qs = Present.objects.get(user=user, date=today)
        except Present.DoesNotExist:
            qs = None

        if qs is not None and qs.present:  # Pastikan absen masuk sudah tercatat
            # Cek apakah user sudah absen pulang
            time_logs = Time.objects.filter(user=user, date=today, out=True)
            if not time_logs.exists():
                if present[person] == True:  # Jika melakukan absensi keluar
                    # Simpan waktu absensi keluar
                    a = Time(user=user, date=today, time=time, out=True)
                    a.save()

                    # Update status keluar pada model Present
                    if time.hour >= 14:  # Jika absensi keluar >= jam 2 siang
                        qs.status_out = "Pulang Tepat Waktu"
                    else:  # Jika absensi keluar < jam 2 siang
                        qs.status_out = "Pulang Tidak Tepat Waktu"
                    qs.save(update_fields=['status_out'])
            else:
                # Jika sudah absen pulang sebelumnya
                print(f"{user.username} sudah absen pulang hari ini.")



def mark_absence_status():
    today = datetime.date.today()
    for qs in Present.objects.filter(date=today):
        time_logs = Time.objects.filter(user=qs.user, date=today, out=True)
        if not time_logs.exists():
            qs.check_out_status = "Tidak Melakukan Absensi"
            qs.save(update_fields=["check_out_status"])
